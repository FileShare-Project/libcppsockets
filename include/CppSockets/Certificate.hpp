...
namespace CppSockets {
    ...
    class x509Name {
        public:
            ...
            [[nodiscard]]
            auto clone() const -> x509Name { return {*this}; }
            ...
            [[nodiscard]]
            auto entry_count() const -> int;
            [[nodiscard]]
            auto get_entry(int loc) const -> x509NameEntry;
            auto delete_entry(int loc) -> x509NameEntry;

            [[nodiscard]]
            auto get_index(int nid, int lastpos = -1) const -> int;
            [[nodiscard]]
            auto get_index(const ASN1_OBJECT *obj, int lastpos = -1) const -> int;
            ...
            [[nodiscard]]
            auto get() const -> X509_NAME * { return m_ptr.get(); }
        private:
            X509_NAME_ptr m_ptr;
    };
    ...
    class x509NameEntry {
        public:
            ...
            [[nodiscard]]
            auto clone() const -> x509NameEntry { return {*this}; }

            void set_object(const ASN1_OBJECT *obj);
            void set_data(int type, const std::u8string &data);

            [[nodiscard]]
            auto get_object() const -> ASN1_OBJECT *;
            [[nodiscard]]
            auto get_data() const -> ASN1_STRING *;

            [[nodiscard]]
            auto get() const -> X509_NAME_ENTRY * { return m_ptr.get(); }
        private:
    ...
    class x509Extension {
        public:
            ...
            [[nodiscard]]
            auto clone() const -> x509Extension { return {*this}; }
            ...
            [[nodiscard]]
            auto get_data() const -> ASN1_OCTET_STRING *;
            [[nodiscard]]
            auto get_object() const -> ASN1_OBJECT *;
            [[nodiscard]]
            auto get_critical() const -> bool;

            [[nodiscard]]
            auto get() const -> X509_EXTENSION * { return m_ptr.get(); }
        private:
    ...
    class x509Certificate {
        public:
            ...
            [[nodiscard]]
            auto clone() const -> x509Certificate { return {*this}; }
            ...
            [[nodiscard]]
            auto get_version() const -> std::int64_t;

            void set_serial_number(std::int64_t serial_number);
            ...
            [[nodiscard]]
            auto extension_count() const -> int;

            [[nodiscard]]
            auto get_extension(std::uint32_t loc) const -> x509Extension;
            [[nodiscard]]
            auto get_extension_by(int nid, int lastpos = - 1) const -> x509Extension;
            [[nodiscard]]
            auto get_extension_by(const ASN1_OBJECT *obj, int lastpos = -1) const -> x509Extension;
            [[nodiscard]]
            auto get_extension_by(bool critical, int lastpos = - 1) const -> x509Extension;

            [[nodiscard]]
            auto self_signed(bool verify_signature) const -> bool;
            [[nodiscard]]
            auto verify(const EVP_PKEY_ptr &pkey) const -> bool;

            void sign(const EVP_PKEY_ptr &pkey, const EVP_MD *digest = EVP_sha256());

            [[nodiscard]]
            auto get() const -> X509 * { return m_ptr.get(); }
        private:
            X509_ptr m_ptr;
    };
